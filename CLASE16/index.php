...
<table class="table">
  ...
  <tbody>
  	<?php
  		...
  		?>
    <tr>
      <!-- los registros se editan en la pagina editar.php -->
      <td><?php echo $info['id']; ?></td>
      <td><?php echo $info['usuario']; ?></td>
      <td><?php echo md5($info['password']); ?></td>
      <td><img width="50px" height="50px" src="<?php echo $info['foto']; ?>"></td>
      <td><a href="editar.php?idEditar=<?php echo $info['id']; ?>">
      <i class="fa fa-pencil-alt"></i></a></td> 
      <td>
          <!-- post -->
          <form method="get" action="enviarmail.php?id=<?php echo $info['id']; ?>">
            <button type="submit" class="btn btn-link">Enviar</button>
          </form>


      </td>  
      <!-- http://localhost:8080/proyecto/editar.php?id= -->
      <!-- http://localhost:8080/proyecto/editar.php?id=0 -->
